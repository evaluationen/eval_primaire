...
class Result extends CI_Controller {
	...
	if($data['result']['gen_certificate']=='0'){
		...
		// save qr 
	$enu=site_url('login/verify_result/'.$rid); // urlencode
        $qrname = $this->base_model->qr_certificat_generate($enu);
        if($qrname)
            $qrname_path = FCPATH ."upload/certificat/".$qrname;
	
        /*$qrname="./upload/certificat/".time().'.jpg';
	$durl="http://chart.googleapis.com/chart?chs=100x100&cht=qr&chl=".$enu."&choe=UTF-8";
        
	copy($durl,$qrname);*/
	
	$certificate_text=$data['result']['certificate_text'];
        if($qrname)
            $certificate_text=str_replace('{qr_code}',"<img width='100px' src='".$qrname_path."'>",$certificate_text);
        
	$certificate_text=str_replace('{email}',$data['result']['email'],$certificate_text);
	$certificate_text=str_replace('{first_name}',  utf8_decode($data['result']['first_name']),$certificate_text);
	$certificate_text=str_replace('{last_name}',utf8_decode($data['result']['last_name']),$certificate_text);
        $certificate_text=str_replace('{birth}', $this->base_model->date_fr($data['result']['birth']),$certificate_text);
	...
	$certificate_text=str_replace('{generated_date}',date('d/m/Y H:i:s',$data['result']['end_time']),$certificate_text);
        
        $result = $this->result_model->get_result_categ($rid);
        $html = '';
		$index = 1;	
        foreach ($result['cat'] as $key=>$r ){
                //$html .= utf8_decode('<br><b color="red">'. strtoupper($r['label'])) . '</b> : ' . round( (($r['correct_ans_result']*100)/$r['correct_ans_base']), 2) . ' % <br>' ;
                $html .= utf8_decode('<br><b color="red">'. strtoupper($r['label'])) . '</b> : ' . round(($r['total_correct_ans']/$r['total_coef'])  *100 ,2). ' % <br>' ;
				
                if(is_array($r['qlist']) && !empty($r['qlist'])){
                    foreach ($r['qlist'] as $q){
                        
                        if(in_array($q['question_type'], array($this->lang->line('short_answer'), $this->lang->line('long_answer')))){
                            $rep = utf8_decode("production écrite de l'élève");
                        }else{
                            $rep = round($q['score']) ? utf8_decode('succès') : utf8_decode('échec');
                        }
                        $key .= $key.') ';
                        
                        $html .= (' Question Num ['.$index.  ']') . ' : '. $rep .'<br>';
                        $index++;
                    }
        ...
        $certificate_text=str_replace('{result}',$html,$certificate_text);
	...
	copy($durl,$qrname);*/
        $qrname = $this->base_model->qr_certificat_generate($enu);
        if($qrname){
            $qrname2= base_url("upload/certificat/".$qrname);
        }
	
	$certificate_text=$data['result']['certificate_text'];
        if($qrname2)
            $certificate_text=str_replace('{qr_code}',"<img width='100px' src='".$qrname2."'>",$certificate_text);
	...
	$data['certificate_text']=$certificate_text;
	  $this->load->view('view_certificate_2',$data);
        ...
        function test(){
            ...
            echo '<pre>';
//            Pour chaque catégorie de questions présente dans le quizz:
//    {titre de la catégorie de question}: {% de réussite dans la catégorie de question}

//Liste des questions de la catégorie et pour chacune:      succès / échec  / production écrite de l'élève si le type de question le demande (réponse longue ou courte).
        $result = $this->result_model->get_result_categ(6);
        $html = '';
        foreach ($result['cat'] as $r ){
                $html .=  $r['label'] . ' : ' . round( (($r['correct_ans_result']*100)/$r['correct_ans_base']), 2) . ' % <br>' ;
                
                if(is_array($r['qlist']) && !empty($r['qlist'])){
                    foreach ($r['qlist'] as $q){
                        
                        if(in_array($q['question_type'], array($this->lang->line('short_answer'), $this->lang->line('long_answer')))){
                            $rep = "production écrite de l'élève si le type de question le demande";
                        }else{
                            $rep = round($q['score']) ? 'succès' : 'échec';
                        }
                        $html .= $q['question'].  ' : ' . $rep .'<br>';
